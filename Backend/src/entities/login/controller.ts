...
export default class LoginController {
  ...
  static async post(req: Request, res: Response) {
    const data: ServiceResult<LoginModel> = await service.post(
      req.body.email,
      req.body.password
    );
  ...
  static async codePost(req: Request, res: Response) {
    const data: ServiceResult<CodeModel> = await service.codePost(
      req.body.secret, req.body.id
    );
    res.status(data.status).json(data);
  }
  ...
  static async getPicture(req: Request, res: Response) {
    const data: ServiceResult<UserModel> = await service.pictureGet(
      req.body.id
    );
    return res.status(data.status).json(data);
  }
